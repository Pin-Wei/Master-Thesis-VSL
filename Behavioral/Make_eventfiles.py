...
for Subj_ID in Subjs:
    ...
    for sheet in sheet_list:
        ...
        Duration = round(Data['Shut-down']-Data['Show-up'], 0)
        # if PREFIX == 'PW':
        ...
        EventOnset.to_csv(PATH +'//sub-{:02d}_task-{:}_run-{:02d}_events.tsv'.format(int(Subj_ID), TASK[int(Subj_ID[0])], R), sep='\t', index=False)
        R += 1
