...
cond_select <- c("Fst", "Snd", "Trd")

rois_name_list  <- c(
  "Vis.Hipp", "Vis.MTL.Hipp", "Hipp_sub", "MTL.Hipp_LR", "Hipp_sub_LR")
  ...
  for ( roi in roi_select ) { 
    for ( cond in cond_list ) {
      ...
      data.comp <- data.frame(Subj   = data.R5$Subj, 
                              Score  = data.R5$Score, 
                              Y.comp = data.R5$Similarity - data.R3$Similarity)
...
if ( plot.2way.per.subj ) {
  for ( roi in roi_select ) {
    for ( sidx in behav$Subj ) {
      ...
      P <- ggerrorplot(subj_df, x = "Run", y = "Similarity", color = "Cond", 
        ...
        ylim(c(min(subset(sub_DF, ROI == roi)$Similarity)-.01, 
               max(subset(sub_DF, ROI == roi)$Similarity)+.01)) + 
...
if ( plot.3way.per.subj ) {
  ...
  for ( sidx in behav$Subj ) {
    ...
    P <- ggerrorplot(subj_df, x = "Run", y = "Similarity", facet.by = "Cond", color = "ROI", 
                     desc_stat = "mean", 
      ...
      ylim(c(min(sub_DF$Similarity)-.01, max(sub_DF$Similarity)+.01)) + 
...
if ( plot.2way.bin.subj ) {
  ...
  for ( roi in roi_select ) {
    P <- ggerrorplot(subset(sub_DF, ROI == roi), 
                     x = "Run", y = "Similarity", facet.by = "Cond", 
...
if ( plot.3way.bin.subj ) {
  ...
  for ( score.bin.label in sort(unique(sub_DF$Score.bin)) ) {
    ...
    P <- ggerrorplot(subb_df, x = "Run", y = "Similarity", 
