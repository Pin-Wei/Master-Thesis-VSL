...
optional.add_argument('-MVM_dataTable',type=str,
                      help='Provide the filepath of the dataTable used.') # My modification
...
if subprocess.check_output(afni_cmd, shell=True, encoding="utf-8")[0] == "0":
    print("")
...
grid_match = subprocess.check_output(grid_cmd, shell=True, encoding="utf-8")
...
if int(grid_match[0]) + int(grid_match[2]) != 2:
    ...
    subprocess.check_output(afni_cmd, shell=True, encoding="utf-8")
else:
...
SubjListT = subprocess.check_output(SubjList_cmd, shell=True, encoding="utf-8")
...
FT_cmd = ("file_tool -show_file_type -show_bad_char -infiles "+SubjTable+
          " -prefix "+CleanTab)
subprocess.check_output(FT_cmd, shell=True, encoding="utf-8")
...
nv = subprocess.check_output(afni_cmd, shell=True, encoding="utf-8")
nv = int(nv.split()[0])
...
dof = subprocess.check_output(dof_cmd, shell=True, encoding="utf-8")
dof = dof.rstrip()
dof = dof.split()

StatType_cmd = "3dAttribute BRICK_STATSYM "+StatFile+"["+ThreshBrik+"]"
StatType = subprocess.check_output(StatType_cmd, shell=True, encoding="utf-8")
...
if StatType == "Ttest":
    ...
    ThrVal = subprocess.check_output(ThrVal_cmd, shell=True, encoding="utf-8")
    ThrVal = ThrVal.rstrip()
elif StatType == "Ftest":
    ThrVal_cmd = "ccalc -expr 'fift_p2t("+p_val+","+dof[3]+","+dof[4]+")'"
    ThrVal = subprocess.check_output(ThrVal_cmd, shell=True, encoding="utf-8")
    ThrVal = ThrVal.rstrip()
elif StatType == "Zscore":
    ThrVal_cmd = "ccalc -expr 'cdf2stat(stat2cdf(x,5,0,0,0),3,0,0,0)'"
    # see: https://afni.nimh.nih.gov/pub/dist/doc/htmldoc/programs/3dttest%2B%2B_sphx.html#other-optionselif StatType == "Zscore":
    ThrVal = subprocess.check_output(ThrVal_cmd, shell=True, encoding="utf-8")
    ThrVal = ThrVal.rstrip()
else:
...
StatLab = subprocess.check_output(StatLab_cmd, shell=True, encoding="utf-8")
...
afni_cmd = ("whereami -coord_file "+ClustTable+"'[13,14,15]' -tab -lpi"+
            " -max_areas 1 -atlas "+atlas+" -atlas MNI_Glasser_HCP_v1.0"+
...
clust_num = subprocess.check_output(afni_cmd, shell=True, encoding="utf-8")
...
if "3dttest++" in hist_all:
    myscript = os.path.join("/media", "data2", "pinwei", "SL_hippocampus", "Combined_group", "code", "HIDDEN_scripts")
    my_HistProg = os.path.join(myscript, "my_ClustExp_HistTable.py")
    my_cmd = ("python "+my_HistProg+" -StatDSET "+StatFile+" -prefix "+prefix+"_GroupTable"+
                "-session "+OutDir)
    # afni_cmd = (HistProg+" -StatDSET "+StatFile+" -prefix "+prefix+"_GroupTable "+
    #             "-session "+OutDir)
    subprocess.check_output(my_cmd, shell=True, encoding="utf-8")

elif "3dMVM" in hist_all:
    my_dataTable_path = args.MVM_dataTable
    my_dataTable = pd.read_csv(my_dataTable_path, sep='\t')
    dataTable_outpath = OutDir+"/"+prefix+"_GroupTable.csv"
    my_dataTable.to_csv(dataTable_outpath, index=False)
