...
if ( dtype == 1 ) {
  roi_select_idx <- c(14, 12, 10, 6)[1]
} else {
  roi_select_idx <- c(15, 13, 11, 7)[1]
}
...
roi_select <- list(
  ...
  # 10:
  c("V1+2", "hLOC+Fus+IT", "MTL"), 
  # 11:
  c("V1+2", "hLOC+Fus+IT", "MTL", "Hippocampus"), 
  # 12:
  ...
  # 14:
  c("V1+2", "hLOC+Fus+IT", "MTL", "Putamen", "Caudate"), 
  # 15:
  c("V1+2", "hLOC+Fus+IT", "MTL", "Hippocampus", "Caudate", "Putamen")
)[roi_select_idx][[1]]
...
## =============================================================================
corr.lm.and.score   <- c(T, F)[2]
corr.diff.and.score <- c(T, F)[2]
plot.2way.per.subj  <- c(T, F)[1]
plot.3way.per.subj  <- c(T, F)[1]
plot.3way.sep.subj  <- c(T, F)[2]
plot.2way.bin.subj  <- c(T, F)[2]
plot.3way.bin.subj  <- c(T, F)[2]
    ...
    for ( sidx in behav$Subj ) {
      ...
      P <- ggerrorplot(subj_df, x = "Run", y = "Y", color = "Cond", 
        ...
        geom_hline(yintercept = 0, color = "black", 
                   linetype = "dotted", size = .5, alpha = .8) + 
...
if ( plot.3way.sep.subj ) {
  sub_DF$Subj <- reorder(sub_DF$Subj, sub_DF$Score, FUN = mean, decreasing = T)
  
  for ( roi in roi_select ) {
    P <- ggline(subset(sub_DF, ROI == roi), 
                x = "Run", y = "Y", facet.by = "Cond", 
                group = "Subj", color = "Subj", 
                xlab = "Exposure runs", ylab = "Intensity", 
                title = roi, 
                point.size = 2, 
      ...
      geom_smooth(aes(x = as.numeric(Run)), 
                  colour = "red", 
    ...
    fig_dir <- file.path(out_dir, "Figure Cond_x_Run_x_ROI (separate subj)")
    if ( ! file.exists(fig_dir) ) { dir.create(fig_dir, recursive=TRUE) }
    
    fig_name <- paste0(roi, ".jpg")
